...
const DASHBOARD_SCRIPT_SRC = "/assets/dashboard.js?v=104";
           ...
           <aside class="inspector" id="workspace-edit-pane" role="tabpanel" aria-labelledby="mobile-tab-edit">${workspaceSlideCreator}
             <details class="inspector-section" data-inspector-section="content" open><summary>内容</summary><div class="inspector-body">
               <form class="editor" data-slide-editor data-versioned-form action="${slidePath}" data-version="${options.project.version}" data-slide-id="${escapeHtml(slide.id)}" data-max-step="${slide.reveal_steps}" data-step-count="${slide.reveal_steps + 1}" data-csrf="${escapeHtml(options.csrfToken)}">
                 ...
                 <div class="actions"><button type="submit">内容を保存</button>${nextSlidePath === null ? "" : `<button class="ghost" type="submit" data-save-next="${nextSlidePath}">保存して次へ</button>`}${slide.role === "content" && (slide.composition === null || slide.composition === undefined) && deck.slides.length < 100 ? `<button class="ghost" type="button" data-slide-split="${slidePath}/split" data-csrf="${escapeHtml(options.csrfToken)}">カーソル位置で2枚に分割</button>` : ""}<span class="version" data-version-label>v${options.project.version}</span></div>
