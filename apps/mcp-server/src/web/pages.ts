...
const DASHBOARD_STYLE = String.raw`
      :root { color-scheme: dark; font-family: Inter, "Noto Sans JP", system-ui, sans-serif; --bg: #090f18; --ink: #eef3fa; --muted: #9aa9bb; --line: #2c3a4e; --panel: #121c2aee; --accent: #9d7bff; }
      * { box-sizing: border-box; }
      body { margin: 0; min-height: 100vh; background: radial-gradient(circle at 15% -10%, #3a285c 0, transparent 36rem), radial-gradient(circle at 85% 0%, #173e57 0, transparent 32rem), var(--bg); color: var(--ink); }
      a { color: inherit; }
      ...
      .project-section-nav a:hover, .project-section-nav a:focus-visible { background: #ffffff12; color: var(--ink); }
      ...
      .landing-nav a { min-height: 2.65rem; padding: .62rem .8rem; border-radius: .65rem; color: #cbd6e4; font-size: .84rem; font-weight: 760; text-decoration: none; }
      .landing-nav a:hover { background: #ffffff10; color: white; }
      ...
      .product-card { padding: 1.15rem; border: 1px solid var(--line); border-radius: .9rem; background: #101a28cc; }
      .product-card h3 { margin: 0 0 .5rem; font-size: 1rem; }
      .product-card p { margin: 0; color: var(--muted); font-size: .88rem; line-height: 1.7; }
      .role-table { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); overflow: hidden; border: 1px solid var(--line); border-radius: 1rem; background: #0d1723cc; }
      .role-table > article { padding: 1.25rem; }
      .role-table > article + article { border-left: 1px solid var(--line); }
      .role-table h3 { margin: 0 0 .7rem; }
      .role-table ul, .plain-list { margin: 0; padding-left: 1.2rem; color: #bdc9d8; line-height: 1.75; }
      .landing-cta { display: flex; align-items: center; justify-content: space-between; gap: 1rem; margin-top: clamp(2.5rem, 7vw, 5rem); padding: clamp(1.25rem, 4vw, 2rem); border: 1px solid #725bc4; border-radius: 1rem; background: linear-gradient(135deg, #33225eee, #14324aee); }
      .landing-cta h2 { margin: 0; font-size: clamp(1.35rem, 4vw, 2rem); }
      .landing-cta p { margin: .45rem 0 0; color: #c7d3e1; line-height: 1.65; }
      .guide-main { max-width: 64rem; }
      ...
      .guide-meta li { padding: .35rem .62rem; border: 1px solid #52647c; border-radius: 999px; color: #c8d4e2; font-size: .78rem; }
      .guide-nav { position: sticky; z-index: 12; top: .5rem; display: flex; gap: .4rem; margin: 1.4rem 0; padding: .45rem; overflow-x: auto; border: 1px solid var(--line); border-radius: .8rem; background: #090f18ed; backdrop-filter: blur(12px); }
      .guide-nav a { flex: 0 0 auto; padding: .58rem .72rem; border-radius: .55rem; color: #cbd6e4; font-size: .78rem; font-weight: 780; text-decoration: none; }
      .guide-nav a:hover { background: #ffffff10; color: white; }
      ...
      .guide-step { display: grid; grid-template-columns: auto minmax(0, 1fr); gap: .85rem; padding: 1rem; border: 1px solid var(--line); border-radius: .85rem; background: #101a28cc; counter-increment: guide-step; }
      ...
      .guide-note { padding: 1rem; border-left: .22rem solid #62d6ff; border-radius: 0 .7rem .7rem 0; background: #112334; color: #c7d3e1; line-height: 1.7; }
      .guide-note strong { color: white; }
      .guide-warning { border-left-color: #f4bd62; background: #2a210d; }
      .guide-client-tabs { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: .65rem; }
      .guide-client-tabs a { padding: 1rem; border: 1px solid var(--line); border-radius: .8rem; background: #101a28; color: inherit; font-weight: 820; text-decoration: none; }
      .guide-client-tabs small { display: block; margin-top: .35rem; color: var(--muted); font-weight: 500; line-height: 1.5; }
      .troubleshooting { width: 100%; border-collapse: collapse; border: 1px solid var(--line); background: #0d1723cc; }
      ...
      .troubleshooting th { width: 32%; color: #e7edf5; }
      .troubleshooting td { color: var(--muted); }
      .official-links { display: flex; flex-wrap: wrap; gap: .5rem; }
      .official-links a { padding: .55rem .7rem; border: 1px solid var(--line); border-radius: .6rem; color: #91ddff; font-size: .8rem; text-decoration: none; }
      ...
      .design-axis-pick[data-design-field="panel_treatment"] .design-axis-wire::before { content: ""; position: absolute; inset: 18% 14%; border: 1px solid #52647c; border-radius: .35rem; background: #26364a; }
      .design-axis-pick[data-design-field="panel_treatment"][data-design-pick="soft"] .design-axis-wire::before { border-color: transparent; background: #31455d; box-shadow: 0 5px 12px #0005; }
      .design-axis-pick[data-design-field="panel_treatment"][data-design-pick="outline"] .design-axis-wire::before { border: 2px solid #9d7bff; background: transparent; }
      .design-axis-pick[data-design-field="panel_treatment"][data-design-pick="raised"] .design-axis-wire::before { border-color: #9d7bff; box-shadow: 4px 4px #9d7bff66; }
      .design-axis-pick[data-design-field="panel_treatment"][data-design-pick="glass"] .design-axis-wire::before { border-color: #ffffff88; background: #ffffff24; box-shadow: 0 6px 14px #0006; }
      ...
      .role-style-summary span { padding: .22rem .48rem; border: 1px solid var(--line); border-radius: 99px; color: #aebcd0; font-size: .68rem; }
      ...
      .workspace-guide { margin-bottom: 1rem; border: 1px solid #35506a; border-radius: .8rem; background: #0a1b29; }
      .workspace-guide > summary { padding: .75rem 1rem; cursor: pointer; color: #bfe6f7; font-weight: 820; }
      ...
      .workspace-guide-body p { margin: 0; padding: .65rem; border: 1px solid #35506a; border-radius: .65rem; color: #c9d5e4; font-size: .78rem; line-height: 1.6; }
      ...
      .review-composer textarea, .review-script textarea { width: 100%; box-sizing: border-box; padding: .65rem; border: 1px solid #52647c; border-radius: .55rem; background: #07101a; color: var(--ink); resize: vertical; }
      ...
      .voice-cue { display: grid; gap: .65rem; padding: .75rem; border: 1px solid #40516a; border-radius: .7rem; background: #07101a; }
      .voice-cue-head { display: flex; align-items: center; justify-content: space-between; gap: .6rem; }
      .voice-cue-head strong { color: #b9ddff; font-size: .78rem; }
      ...
      .voice-composed { margin: 0; padding: .65rem .75rem; border-left: 3px solid var(--accent); background: #0c1724; color: #d9e3ef; font-size: .82rem; line-height: 1.7; white-space: pre-wrap; overflow-wrap: anywhere; }
      .voice-pause-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: .55rem; }
      .voice-howto { border: 1px solid #35506a; border-radius: .75rem; background: #0a1b29; }
      .voice-howto > summary { padding: .75rem; cursor: pointer; color: #bfe6f7; font-weight: 800; }
      .voice-howto-body { display: grid; gap: .55rem; padding: 0 .75rem .75rem; color: #bdc9d8; font-size: .8rem; line-height: 1.65; }
      ...
      .voice-filter { position: sticky; z-index: 12; top: .5rem; display: grid; gap: .45rem; margin-inline: -.55rem; padding: .55rem; border: 1px solid #35465d; border-radius: .8rem; background: #0b1420f2; box-shadow: 0 .45rem 1rem #02060c66; backdrop-filter: blur(14px); }
      ...
      @media (max-width: 72rem) { .slide-workspace { grid-template-columns: minmax(9rem, 13rem) minmax(0, 1fr); } .inspector { grid-column: 1 / -1; max-height: none; } .workspace-guide-body { grid-template-columns: repeat(2, minmax(0, 1fr)); } .review-workspace { grid-template-columns: minmax(0, 1fr) minmax(18rem, 21rem); } .review-filmstrip { position: static; grid-column: 1 / -1; max-height: none; overflow: visible; scrollbar-gutter: auto; } .review-filmstrip-list { grid-auto-flow: column; grid-auto-columns: minmax(12rem, 14rem); padding-bottom: .4rem; overflow-x: auto; overscroll-behavior-inline: contain; scroll-snap-type: x proximity; scrollbar-width: thin; } .review-filmstrip-list .filmstrip-link { scroll-snap-align: start; } .review-comments { grid-column: 2; } }
  ...
  return new Response(
    shell(
      ...
      `<header class="site-header"><a class="brand" href="/">最自由研究</a><nav class="landing-nav" aria-label="案内"><a href="#about">できること</a><a href="#roles">使い分け</a><a class="button ghost" href="/guide">はじめかた</a></nav></header>
       ...
       <section class="landing-section" id="roles"><div class="landing-section-head"><p class="eyebrow">Two surfaces</p><h2>AIとWeb UIには、得意な仕事があります</h2></div><div class="role-table"><article><h3>Codex・Claudeで進めること</h3><ul><li>テーマを一緒に探す</li><li>問い、方法、記録、発見を整理する</li><li>発表の流れや各スライドを構成する</li><li>対象を読んで小さな単位で修正する</li></ul></article><article><h3>ブラウザで進めること</h3><ul><li>Twitchで本人確認する</li><li>画像を追加し、実際の表示を確認する</li><li>文言、見た目、VOICEVOXを仕上げる</li><li>固定プレビューを確認し、公開する</li></ul></article></div></section>
       ...
       <section class="landing-cta"><div><h2>初回設定は5〜10分ほど</h2><p>Codex、Claude Code、ClaudeのWeb／Desktopそれぞれの手順を、画面どおりに案内します。</p></div><a class="button" href="/guide">はじめかたを見る</a></section>
       </main>`
    ),
    { headers: headers() }
  );
    ...
    shell(
      ...
      `<header class="site-header"><a class="brand" href="/">最自由研究</a><nav class="landing-nav" aria-label="案内"><a href="/">プロダクト説明</a><a class="button ghost" href="/login">Twitchでログイン</a></nav></header>
       <main class="guide-main" id="main-content" tabindex="-1">
         ...
         <nav class="guide-nav" aria-label="このページの目次"><a href="#before">準備</a><a href="#codex">Codex</a><a href="#claude-code">Claude Code</a><a href="#claude-web">Claude Web／Desktop</a><a href="#first-research">最初の研究</a><a href="#trouble">困ったとき</a></nav>
         <section class="guide-section" id="before"><p class="eyebrow">Before you start</p><h2>最初に用意するもの</h2><div class="product-grid"><article class="product-card"><h3>利用資格のあるTwitchアカウント</h3><p>${escapeHtml(options.broadcasterLogin)}を${options.minFollowDays}日以上フォロー中、または現在サブスク中のアカウントを使います。</p></article><article class="product-card"><h3>対応するAIクライアント</h3><p>Codex、Claude Code、またはClaudeのカスタムコネクタなど、Remote MCPのHTTP接続に対応する環境が必要です。</p></article><article class="product-card"><h3>ブラウザ</h3><p>AI側の認証中にTwitchが開きます。Web UIへ先にログインしていても、AI接続の許可は別に一度行います。</p></article></div><div class="guide-note"><strong>AIサービスの契約は本サービスに含まれません。</strong> お使いのCodexまたはClaudeでRemote MCPが利用できることを確認してください。</div><div class="actions"><a class="button" href="/login">先にWeb UIへログイン</a></div></section>
         <section class="guide-section" id="choose"><p class="eyebrow">Choose your client</p><h2>使う環境を選ぶ</h2><div class="guide-client-tabs"><a href="#codex">Codex<small>CLI・IDE・設定を共有するCodex環境</small></a><a href="#claude-code">Claude Code<small>ターミナルから使う場合</small></a><a href="#claude-web">Claude Web／Desktop<small>カスタムコネクタを使う場合</small></a></div></section>
...
export function projectDetailPage(options: {
  ...
  const publicationPanel = `<section class="panel publish-state" id="publication" tabindex="-1" data-publication>
    ...
    ${publicationEvents}
    <div class="actions">
      <button type="button" data-create-preview="/api/projects/${escapeHtml(options.project.project_id)}/previews" data-version="${options.project.version}" data-can-preview="${String(slides.length > 0 && !voiceIncomplete)}" data-csrf="${escapeHtml(options.csrfToken)}"${slides.length === 0 || voiceIncomplete ? " disabled" : ""}>現在の下書きをプレビュー</button>
...
export function slideWorkspacePage(options: {
  ...
  const roleStyleEditor = activeTemplate === undefined ? "" : `<fieldset data-role-style-editor data-role-styles="${escapeHtml(JSON.stringify(activeTemplate.role_styles ?? {}))}" data-role-style-base="${escapeHtml(JSON.stringify({ background: activeTemplate.background, surface: activeTemplate.surface, foreground: activeTemplate.foreground, muted: activeTemplate.muted, accent: activeTemplate.accent, motif_color: activeTemplate.motif_color ?? activeTemplate.accent }))}"><legend>役割ごとのデザイン差分</legend><p class="inherit-note">基本デザインを複製せず、表紙、章扉、比較、結果などに必要な差分だけを設定します。ここで選んだ役割として現在のスライドへ即時プレビューします。</p><div class="editor-grid"><label>編集する役割<select name="role_style_role">${Object.entries(SLIDE_ROLE_LABELS).map(([value, label]) => `<option value="${value}"${selectedRole === value ? " selected" : ""}>${label}</option>`).join("")}</select></label><label class="check-label"><input name="role_style_enabled" type="checkbox"${selectedRoleStyle === undefined ? "" : " checked"}>この役割に差分を設定</label></div><div class="role-style-summary" data-role-style-summary>${configuredRoleSummary || "<span>差分なし</span>"}</div><div class="role-style-controls"><div class="editor-grid"><label>領域配置<select name="role_style_region_layout"><option value="">基本を継承</option>${[["single", "単一"], ["sidebar-right", "右補足"], ["sidebar-left", "左補足"], ["lower-third", "下段補足"], ["split", "左右均等"], ["top-band", "上段補足"], ["focus", "中央集中"]].map(([value, label]) => `<option value="${value}"${selectedRoleStyle?.region_layout === value ? " selected" : ""}>${label}</option>`).join("")}</select></label><label>背景モチーフ<select name="role_style_motif"><option value="">基本を継承</option>${Object.entries(MOTIF_LABELS).map(([value, label]) => `<option value="${value}"${selectedRoleStyle?.motif === value ? " selected" : ""}>${label}</option>`).join("")}</select></label><label>見出し処理<select name="role_style_heading_treatment"><option value="">基本を継承</option>${Object.entries(HEADING_TREATMENT_LABELS).map(([value, label]) => `<option value="${value}"${selectedRoleStyle?.heading_treatment === value ? " selected" : ""}>${label}</option>`).join("")}</select></label><label>カード・補足欄<select name="role_style_panel_treatment"><option value="">基本を継承</option>${Object.entries(PANEL_TREATMENT_LABELS).map(([value, label]) => `<option value="${value}"${selectedRoleStyle?.panel_treatment === value ? " selected" : ""}>${label}</option>`).join("")}</select></label></div><div class="editor-grid">${roleStyleColor("background", "背景", selectedRoleStyle?.background, activeTemplate.background)}${roleStyleColor("surface", "カード・補足面", selectedRoleStyle?.surface, activeTemplate.surface)}${roleStyleColor("foreground", "本文", selectedRoleStyle?.foreground, activeTemplate.foreground)}${roleStyleColor("muted", "補助文字", selectedRoleStyle?.muted, activeTemplate.muted)}${roleStyleColor("accent", "アクセント", selectedRoleStyle?.accent, activeTemplate.accent)}${roleStyleColor("motif_color", "モチーフ色", selectedRoleStyle?.motif_color, activeTemplate.motif_color ?? activeTemplate.accent)}<label>モチーフの濃さ<input name="role_style_motif_opacity" type="number" min="0" max="0.5" step="0.05" value="${selectedRoleStyle?.motif_opacity ?? ""}" placeholder="基本を継承"></label></div><p class="quality-status" data-role-contrast-status data-level="${roleMainContrast !== null && roleSidebarContrast !== null && roleMainContrast >= 4.5 && roleSidebarContrast >= 4.5 ? "ok" : "warning"}">この役割の本文 ${roleMainContrast?.toFixed(1)}:1 · 補足 ${roleSidebarContrast?.toFixed(1)}:1</p></div></fieldset>`;
    ...
    ? [selectedNarrationSegment]
        .map((segment) => {
          ...
          const cueMarkup = voiceCues.map((cue, index) => {
            ...
            return `<fieldset class="voice-cue" data-voice-cue><legend class="sr-only">発話ブロック ${index + 1}</legend><div class="voice-cue-head"><strong data-voice-cue-label>発話 ${index + 1}</strong><button type="button" class="ghost danger" data-remove-voice-cue${voiceCues.length === 1 ? " disabled" : ""}>削除</button></div><input type="hidden" name="cue_id" value="${escapeHtml(cue.id)}"><label>この声で読む文<textarea name="cue_text" maxlength="500" required>${escapeHtml(cue.text)}</textarea></label><div class="editor-grid"><label>声<select name="cue_profile_id">${cueProfileOptions}</select></label><label>直後の休符（秒）<input name="cue_pause_after_seconds" type="number" min="0" max="10" step="0.1" value="${(cue.pause_after_ms ?? 0) / 1000}"></label></div><div class="voice-cue-preset" role="group" aria-label="話し方のプリセット"><button type="button" class="ghost" data-voice-cue-preset="standard">標準</button><button type="button" class="ghost" data-voice-cue-preset="emphasis">強調</button><button type="button" class="ghost" data-voice-cue-preset="calm">落ち着き</button><button type="button" class="ghost" data-voice-cue-preset="quick">早口</button></div><div class="tuning-grid"><label>速度<input name="cue_speedScale" type="number" min="${VOICEVOX_TUNING_LIMITS.speedScale.min}" max="${VOICEVOX_TUNING_LIMITS.speedScale.max}" step="0.01" value="${cue.voice_tuning?.speedScale ?? ""}" placeholder="継承"></label><label>高さ<input name="cue_pitchScale" type="number" min="${VOICEVOX_TUNING_LIMITS.pitchScale.min}" max="${VOICEVOX_TUNING_LIMITS.pitchScale.max}" step="0.01" value="${cue.voice_tuning?.pitchScale ?? ""}" placeholder="継承"></label><label>抑揚<input name="cue_intonationScale" type="number" min="${VOICEVOX_TUNING_LIMITS.intonationScale.min}" max="${VOICEVOX_TUNING_LIMITS.intonationScale.max}" step="0.01" value="${cue.voice_tuning?.intonationScale ?? ""}" placeholder="継承"></label></div></fieldset>`;
          }).join("");
          ...
          const cueTemplate = `<template data-voice-cue-template><fieldset class="voice-cue" data-voice-cue><legend class="sr-only">新しい発話ブロック</legend><div class="voice-cue-head"><strong data-voice-cue-label>発話</strong><button type="button" class="ghost danger" data-remove-voice-cue>削除</button></div><input type="hidden" name="cue_id" value=""><label>この声で読む文<textarea name="cue_text" maxlength="500" required placeholder="声やトーンを変える位置で文章を分けます"></textarea></label><div class="editor-grid"><label>声<select name="cue_profile_id">${cueTemplateOptions}</select></label><label>直後の休符（秒）<input name="cue_pause_after_seconds" type="number" min="0" max="10" step="0.1" value="0"></label></div><div class="voice-cue-preset" role="group" aria-label="話し方のプリセット"><button type="button" class="ghost" data-voice-cue-preset="standard">標準</button><button type="button" class="ghost" data-voice-cue-preset="emphasis">強調</button><button type="button" class="ghost" data-voice-cue-preset="calm">落ち着き</button><button type="button" class="ghost" data-voice-cue-preset="quick">早口</button></div><div class="tuning-grid"><label>速度<input name="cue_speedScale" type="number" min="${VOICEVOX_TUNING_LIMITS.speedScale.min}" max="${VOICEVOX_TUNING_LIMITS.speedScale.max}" step="0.01" placeholder="継承"></label><label>高さ<input name="cue_pitchScale" type="number" min="${VOICEVOX_TUNING_LIMITS.pitchScale.min}" max="${VOICEVOX_TUNING_LIMITS.pitchScale.max}" step="0.01" placeholder="継承"></label><label>抑揚<input name="cue_intonationScale" type="number" min="${VOICEVOX_TUNING_LIMITS.intonationScale.min}" max="${VOICEVOX_TUNING_LIMITS.intonationScale.max}" step="0.01" placeholder="継承"></label></div></fieldset></template>`;
          return `<form class="voice-segment editor" id="narration-segment-${segment.at}" data-segment-editor data-segment-preview data-versioned-form action="${slidePath}/narration/segments/${segment.at}" data-version="${options.project.version}" data-slide-id="${escapeHtml(slide.id)}" data-segment-at="${segment.at}" data-effective-tuning="${escapeHtml(JSON.stringify(effectiveTuning))}" data-profile-tunings="${escapeHtml(JSON.stringify(profileTunings))}" data-profile-catalogs="${escapeHtml(JSON.stringify(profileCatalogIds))}" data-step-duration="${stepDuration}" data-csrf="${escapeHtml(options.csrfToken)}">
            ...
            <fieldset><legend>文中の声とトーン</legend><div class="voice-cue-list" data-voice-cue-list>${cueMarkup}</div>${cueTemplate}<button type="button" class="ghost" data-add-voice-cue${voiceCues.length >= 8 ? " disabled" : ""}>＋ 声を変える位置を追加</button><small class="inherit-note">最大8ブロック。各ブロックの文章は続けて画面へ表示され、声・速度・高さ・抑揚だけを切り替えられます。</small></fieldset>
       ...
       <main id="main-content" tabindex="-1" class="workspace-main">
         ...
         <div class="slide-workspace" data-workspace-asset-urls="${escapeHtml(JSON.stringify(workspaceAssetUrls))}" data-selected-component="${escapeHtml(selectedComponentId ?? "")}" data-selected-narration="${selectedNarrationSegment?.at ?? ""}">
           ...
           <aside class="inspector" id="workspace-edit-pane">${workspaceSlideCreator}
             <div class="mobile-inspector-tabs" role="tablist" aria-label="編集項目" hidden><button class="ghost" id="inspector-tab-content" type="button" role="tab" data-inspector-pane="content" aria-controls="inspector-content">内容</button><button class="ghost" id="inspector-tab-design" type="button" role="tab" data-inspector-pane="design" aria-controls="inspector-design">デザイン</button><button class="ghost" id="inspector-tab-narration" type="button" role="tab" data-inspector-pane="narration" aria-controls="inspector-narration">読み上げ</button><button class="ghost" id="inspector-tab-structure" type="button" role="tab" data-inspector-pane="structure" aria-controls="inspector-structure">構造</button><button class="ghost" id="inspector-tab-quality" type="button" role="tab" data-inspector-pane="quality" aria-controls="inspector-quality">品質確認</button></div>
