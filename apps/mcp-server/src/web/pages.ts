  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .font-pick[data-font-pick="gothic"] { font-family: "BIZ UDPGothic", "Yu Gothic", sans-serif; }
      .font-pick[data-font-pick="rounded"] { font-family: "M PLUS Rounded 1c", ui-rounded, sans-serif; }
      .font-pick[data-font-pick="mincho"] { font-family: "Noto Serif JP", "Yu Mincho", serif; }
      ...
      @media (max-width: 48rem) { .hero, .detail-grid, .editor-grid, .slide-workspace, .tuning-grid, .voice-flow, .voice-hero, .journey-next, .setup-steps, .voice-preset-fields { grid-template-columns: 1fr; } .dashboard-tools { align-items: stretch; flex-direction: column; } .dashboard-filter { justify-content: flex-start; } .editor label.wide { grid-column: auto; } .filmstrip { display: flex; max-height: none; overflow-x: auto; } .filmstrip-link { min-width: 12rem; } .inspector { grid-column: auto; } .voice-stats, .journey-steps { grid-template-columns: repeat(2, minmax(0, 1fr)); } .voice-next { position: static; } }
...
export function projectDetailPage(options: {
  ...
  const publicationPanel = `<section class="panel publish-state" id="publication" data-publication>
    ...
    <div class="actions">
      ...
      <button class="ghost" type="button" data-publish-preview="/api/projects/${escapeHtml(options.project.project_id)}/publish" data-revision="${escapeHtml(preview?.revision_id ?? "")}" data-csrf="${escapeHtml(options.csrfToken)}" data-duration-valid="${String(durationWithinLimit)}" data-published-current="${String(publishedCurrent)}"${previewCurrent && durationWithinLimit && !publishedCurrent ? "" : " disabled"}>${publishedCurrent ? "この版は公開済み" : "確認した版を公開"}</button>
    </div>
         ...
         <div class="voice-flow">
           <div class="voice-column">
             ...
             <section class="panel voice-step"><div class="voice-step-head"><span class="voice-step-number">2</span><div><h2>不足分を生成する</h2><p>設定や原稿が変わった区間だけを生成します。生成済みの音声は再利用します。</p></div></div>
               <div class="voice-stats"><div class="voice-stat"><span>原稿</span><strong data-voice-total>${summary.total}</strong></div><div class="voice-stat ready"><span>生成済み</span><strong data-voice-ready>${summary.ready}</strong></div><div class="voice-stat pending"><span>要生成<small>失敗含む</small></span><strong data-voice-needed>${summary.needs_generation}</strong></div><div class="voice-stat"><span>失敗</span><strong data-voice-failed>${summary.failed}</strong></div></div>
