...
- スライドごとに想定時間を設定する
- 全画面表示する
- 読み上げ原稿をADV会話枠・実況字幕・全文追従・映像字幕・最小表示の5形式で画面に表示する
...
| 読み上げ音量 | 右下の `音量` スライダー。フォーカス中は矢印キーでも調整可能 |
| 自動送り ON／OFF | `A` または右下の `自動` ボタン |
...
右下の音声ボタンは今後の自動読み上げを止めるミュート切替です。`自動` の自動送りをオンにすると、音声の終了後、またはSTEPの想定時間後に次の段階へ進み、そのまま発表を連続再生します。初期状態では自動読み上げがオン、自動送りがオフです。
...
4. `A` キーまたは `自動` ボタンで自動送りをオンにし、音声終了後に次へ進むことを確認する
